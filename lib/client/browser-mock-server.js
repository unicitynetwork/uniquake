    ...
    async createGameStateToken(broadcast = true) {
      ...
      try {
        ...
        if (broadcast && token) {
          ...
          // Include additional game state information in the broadcast
          // This way the client can display the server's actual values
          this.broadcast({
            type: 'game:state:token',
            tokenFlow: tokenFlow,
            frame: this.serverState.gameState.frame,
            // Add extra state information
            serverInfo: {
              playerCount: Object.keys(this.serverState.gameState.players || {}).length,
              itemCount: Object.keys(this.serverState.gameState.items || {}).length,
