  ...
  class GameIntegration {
    ...
    setupClientLogic() {
      ...
      client.on('tokenUpdate', (tokenStatus) => {
        // Make sure we have valid token data before accessing properties
        if (tokenStatus && tokenStatus.type === 'game_state_verification') {
          // This is a game state verification event, handle differently
          
          // Log any server info for debugging
          if (tokenStatus.serverInfo) {
            this.log(`[DIRECT_FIX] Game integration received server info: ${JSON.stringify(tokenStatus.serverInfo)}`);
          }
          
          // Don't emit token updates that are too frequent - limit to once per second
          const lastUpdate = this._lastTokenUpdate || 0;
          const now = Date.now();
          
          if (now - lastUpdate > 1000) {
            this._lastTokenUpdate = now;
            
            const eventData = {
              type: 'game_state_verification',
              result: tokenStatus.result,
              frame: tokenStatus.frame,
              timestamp: now
            };
            
            // Pass along server info if available
            if (tokenStatus.serverInfo) {
              eventData.serverInfo = tokenStatus.serverInfo;
              this.log(`[DIRECT_FIX] Passing server info in token event: ${JSON.stringify(tokenStatus.serverInfo)}`);
            }
            
            this.emit('tokenEvent', eventData);
          }
        } else if (tokenStatus && tokenStatus.tokens) {
          // This is a token inventory update
          this.emit('tokenEvent', {
          ...
          // Fallback for other types of token updates
          this.emit('tokenEvent', {
            type: 'token_update',
            data: tokenStatus
          });
    ...
    async updateGameState(gameState) {
      ...
      try {
        ...
        // Enhanced debugging for game state tokens
        ...
        if (typeof window !== 'undefined' && window.normalizeGameState && window.hashGameState) {
          const normalizedState = window.normalizeGameState(gameState);
          const stateHash = window.hashGameState(gameState);
          this.debug(`Game state normalized frame: ${normalizedState.frame}, hash: ${stateHash}`);
        }
