...
(function(window) {
  ...
  class BrowserMockClient {
    ...
    handlePlayerScoreUpdateWithTimestampCheck(message) {
      ...
      // Check if we have recent master server data
      const latestScore = this.clientState.latestPlayerScore;
      const masterRequestTime = this.clientState.lastMasterScoreRequest || 0;
      const currentTime = Date.now();
      
      // If master server request was made recently (within last 10 seconds), ignore server page data
      if (currentTime - masterRequestTime < 10000) {
        this.debug('Ignoring server page data - recent master request detected');
        return;
      }
      
      // If we have recent score data from master (within last 15 seconds), ignore server page data
      if (latestScore && latestScore.lastUpdate && (currentTime - latestScore.lastUpdate) < 15000) {
        this.debug('Ignoring server page data - recent master data available');
        return;
      }
      
      // If server page data is too old (more than 30 seconds), ignore it
      if (message.timestamp && (currentTime - message.timestamp) > 30000) {
        this.debug('Ignoring server page data - too old');
        return;
      }
      
      // Fallback: use server page data when master is failing
      ...
      // Process the score update normally
