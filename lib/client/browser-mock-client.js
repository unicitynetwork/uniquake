...
(function(window) {
  ...
  class BrowserMockClient {
    ...
    updateOrangeBannerFromServerRegistry() {
      ...
      // Find the current server we're connected to
      let restartCountdown = null;
      
      // Try to get countdown from client state if we know which server we're connected to
      if (this.clientState?.connectedServerId && window.serverTimers) {
        const serverTimer = window.serverTimers[this.clientState.connectedServerId];
        if (serverTimer && serverTimer.state === 'game_over') {
          const currentTime = Date.now();
          const elapsedSeconds = (currentTime - serverTimer.localStartTime) / 1000;
          const currentCountdown = Math.max(0, Math.ceil(serverTimer.serverRestartCountdown - elapsedSeconds));
          restartCountdown = currentCountdown > 0 ? currentCountdown : null;
        }
      }
      
      // If we don't have specific server data, try to find any game_over server with countdown
      if (restartCountdown === null && window.serverTimers) {
        for (const [peerId, timerData] of Object.entries(window.serverTimers)) {
          if (timerData.state === 'game_over' && timerData.serverRestartCountdown > 0) {
            const currentTime = Date.now();
            const elapsedSeconds = (currentTime - timerData.localStartTime) / 1000;
            const currentCountdown = Math.max(0, Math.ceil(timerData.serverRestartCountdown - elapsedSeconds));
            if (currentCountdown > 0) {
              restartCountdown = currentCountdown;
              break;
            }
          }
        }
