...
(function(window) {
  ...
  class BrowserMockClient {
    ...
    /**
     * Show server restart notification with countdown
     */
    showServerRestartNotification(message) {
      ...
      this.log('🔄 Server restart notification displayed');
    }
    
    /**
     * Update restart countdown display
     */
    updateRestartCountdown(remainingTime) {
      const countdownEl = document.getElementById('restart-countdown');
      if (countdownEl) {
        const seconds = Math.ceil(remainingTime / 1000);
        countdownEl.innerHTML = `⏱️ Next match in: ${seconds}s`;
      }
    }
    ...
    /**
     * Handle server restart complete - restart the game iframe
     */
    handleServerRestartComplete(message) {
      ...
      completeOverlay.innerHTML = `
        ...
        <div>Reconnecting to new match...</div>
      `;
      
      document.body.appendChild(completeOverlay);
      
      // Restart the game iframe to connect to new server
      setTimeout(() => {
