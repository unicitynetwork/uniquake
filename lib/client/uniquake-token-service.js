  ...
  class UniQuakeTokenService {
    ...
    async createGameStateToken(gameState) {
      try {
        ...
        // Use the normalized game state and hash it
        const stateHash = this.hashGameState(gameState);
        
        // Create immutable data with ONLY the essential information for verification
        // IMPORTANT: We don't need to generate a random ID if gameId is missing
    ...
    async updateGameStateToken(stateToken, newState) {
      try {
        ...
        if (currentFrame > 0 && currentFrame % this.resetFrameInterval === 0) {
          this.debug(`Resetting token at frame ${currentFrame} to prevent size growth`);
        ...
        // Create a message with the new state hash - ensure consistency with createGameStateToken
        const message = {
          state_hash: stateHash,
          timestamp: Date.now(),
        ...
        this.debug(`Creating transaction for token ${tokenToUpdate.tokenId}`);
        ...
        try {
          ...
          this.debug(`Created data hash: ${dataHash.substring(0, 10)}...`);
          ...
          this.debug(`Transaction created, now exporting token flow with transaction`);
    ...
    async verifyGameStateToken(tokenFlow) {
      try {
        ...
        if (!tokenFlow) {
          return { verified: false, error: 'No token flow provided' };
        }
    ...
    hashGameState(gameState) {
      // Create deterministic serialization directly from gameState
      // The serializeGameState method already handles normalization
      ...
      // Log the hash result
      if (this.debugMode) {
        console.log(`Generated hash for frame ${gameState?.frame || 0}: ${hash}`);
      }
      
      // Make this function available globally for consistent hashing
      window.hashGameState = (state) => this.hashGameState(state);
      
      return hash;
    }
    
    /**
     * Normalize a game state to ensure consistent hashing
     * @param {Object} gameState - Raw game state
     * @returns {Object} - Normalized game state
     */
    normalizeGameState(gameState) {
      // IMPORTANT: We need to create an EXACT match between client and server
      // So we'll create a MINIMAL representation with ONLY frame and gameId
      // This ensures that no extra fields like timestamps or item counts can cause mismatches
      
      // Create a bare minimum state for hash verification
      const minimalState = {
        // Order matters for consistency in JSON serialization
        frame: parseInt(gameState?.frame || 0, 10), // Ensure it's an integer
        gameId: String(gameState?.gameId || '')     // Ensure it's a string
        // Deliberately exclude players, items, and scores
        // These can be different between client/server
      };
      ...
      // Make this function available globally for consistent normalization
      window.normalizeGameState = this.normalizeGameState;
    ...
    serializeGameState(gameState) {
      // First, ensure we normalize the game state using our standardized function
      const normalizedState = this.normalizeGameState(gameState);
      
      // EXTREMELY simple serialization for consistency
      // Since our normalizedState only has two fields in a fixed order (frame, gameId),
      // we can use basic JSON.stringify which will be consistent across all platforms
    ...
    async resetGameStateToken(newState) {
      try {
        this.debug(`Creating fresh token for frame ${newState.frame || 0} (discarding old token)`);
        ...
        this.debug(`Successfully reset token at frame ${newState.frame || 0}`);
        return newToken;
      } catch (error) {
