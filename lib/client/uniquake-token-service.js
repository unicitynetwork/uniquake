...
(function(window) {
  ...
  class UniQuakeTokenService {
    ...
    serializeGameState(gameState) {
      ...
      // IMPORTANT: Only include fields that are essential for gameplay verification
      // Exclude any non-deterministic or display-only properties
      const essentialState = {
        gameId: gameState.gameId || '',
        frame: gameState.frame || 0,
        
        // NOTE: For deterministic hashing, we need to remove timestamp
        // or normalize it to a standard value based on frame number
        // This ensures client and server hashes match even with time differences
        
        // Use players without position (which changes frequently and isn't crucial for verification)
        players: {}
      };
      
      // Process players to only include essential data
      if (gameState.players) {
        Object.keys(gameState.players).forEach(playerId => {
          const player = gameState.players[playerId];
          essentialState.players[playerId] = {
            id: player.id,
            name: player.name,
            health: player.health,
            // Exclude position which changes frequently
            // Exclude joinTime which is non-deterministic
          };
        });
      }
      
      // Add scores if present (important for verification)
      if (gameState.score && Object.keys(gameState.score).length > 0) {
        essentialState.score = {};
        Object.keys(gameState.score).forEach(playerId => {
          essentialState.score[playerId] = gameState.score[playerId];
        });
      }
      
      // Log what we're serializing for debugging
      this.debug('Serializing game state for hashing:', JSON.stringify(essentialState));
