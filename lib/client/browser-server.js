/**
 * Browser Game Server for real WebRTC P2P connections
 * 
 * This provides real game server functionality in a web browser environment
 */
...
(function(window) {
  ...
  /**
   * Server Channel for the browser environment
   */
  class ServerChannel {
  ...
  class BrowserServer {
     ...
     */
    async init() {
      this.log('Initializing browser server...');
    ...
    handleConnectionRequest(message) {
      ...
      if (identity) {
        ...
        this.log(`No identity provided for client ${clientId}, will use network ID as username until identity update`);
      }
      
      // Create a new channel for this client
      const channel = new ServerChannel(clientId);
    ...
    /**
     * Handle server list response for status updates
     */
    handleServerList(message) {
      const servers = message.servers || [];
      
      // Call the browser callback if available
      if (window.handleServerListForStatus) {
        window.handleServerListForStatus(servers);
      }
    }
    ...
    setupChannel(channel, clientId) {
      channel.onopen = () => {
        ...
        const welcomeMessage = {
          type: 'welcome',
          message: `Welcome to ${this.serverInfo.name}! Please wait while we verify your identity...`,
          serverInfo: {
            ...this.serverInfo,
            tokenEnabled: !!this.serverState.tokenService,
            entryFee: this.config.entryFee,
          ...
          switch (message.type) {
            ...
            case 'identity:update':
              ...
              if (message.identity && message.identity.pubkey) {
                const client = this.serverState.clients.get(clientId);
                if (client) {
                  const newPubkey = message.identity.pubkey;
                  
                  // Check for duplicate connections with the same pubkey
                  const duplicateConnections = [];
                  this.serverState.clients.forEach((otherClient, otherClientId) => {
                    if (otherClientId !== clientId && otherClient.pubkey === newPubkey) {
                      duplicateConnections.push(otherClientId);
                    }
                  });
                  
                  // Close all duplicate connections
                  if (duplicateConnections.length > 0) {
                    this.log(`Found ${duplicateConnections.length} duplicate connections for pubkey ${newPubkey.substring(0, 10)}..., closing them`);
                    
                    duplicateConnections.forEach(duplicateClientId => {
                      this.log(`Closing duplicate connection ${duplicateClientId} for pubkey ${newPubkey.substring(0, 10)}...`);
                      this.handleClientDisconnect(duplicateClientId);
                    });
                  }
                  
                  // Update this client's identity
                  client.pubkey = newPubkey;
                  ...
                  // Check if this player (by pubkey) has already paid entry tokens
                  if (client.entryTokenReceived) {
                    paymentRequired = false;
                    isRejoining = true;
                    this.log(`Player ${client.pubkey.substring(0, 10)}... has already paid - no payment required for rejoin`);
                  }
                  ...
                  const paymentMessage = {
                    ...
                    message: isRejoining 
                      ...
                      : `Hello ${client.username}! Entry fee: ${this.config.entryFee} token${this.config.entryFee !== 1 ? 's' : ''} required to join.`,
                    serverInfo: {
                  ...
                  this.log(`Sending payment requirement to client ${clientId}: paymentRequired=${paymentRequired}, isRejoining=${isRejoining}`);
                  this.sendToClient(clientId, paymentMessage);
                  
                  // Trigger game state update to reflect the new pubkey in player information
                  if (this.serverState.tokenService) {
                    this.createGameStateToken(false); // Update without broadcasting
                  }
                  ...
                  // Emit status update to refresh client list in UI
                  this.emitStatusUpdate();
                }
              }
              break;
    ...
    async handleGameStateTokenRequest(clientId) {
      ...
      try {
        ...
        if (this.serverState.tokenService.lastStateToken) {
          ...
          // Create server state info for the client
          const serverStateInfo = {
            playerCount: Object.keys(this.serverState.gameState.players || {}).length,
            itemCount: Object.keys(this.serverState.gameState.items || {}).length || 0,
            timestamp: Date.now() // Server's current time
          };
          
          // Log what we're sending
          this.log(`[DIRECT_FIX] Sending server info to client ${clientId}: ${JSON.stringify(serverStateInfo)}`);
          
          // Send to the requesting client with server info
          this.sendToClient(clientId, {
            type: 'game:state:token',
            tokenFlow: tokenFlow,
            frame: this.serverState.gameState.frame,
            serverInfo: serverStateInfo // Always include server info
          });
    ...
    async createGameStateToken(broadcast = true) {
      ...
      try {
        ...
        if (this.gameInstance) {
          ...
          // Always log the frame number for debugging
          this.debug(`Syncing frames - Current game state frame: ${currentGameState.frame}`);
          
          // If we have global normalize and hash functions, use them for debugging
          if (typeof window !== 'undefined' && window.normalizeGameState && window.hashGameState) {
            this.debug('Using globally shared normalization and hash functions for consistency');
            const normalizedState = window.normalizeGameState(currentGameState);
            const stateHash = window.hashGameState(currentGameState);
            this.debug(`Normalized state frame: ${normalizedState.frame}, hash: ${stateHash}`);
          }
          
          // Start with the game's current state as the base
          this.serverState.gameState = {
            gameId: currentGameState.gameId,
            frame: currentGameState.frame,
            timestamp: currentGameState.timestamp,
            players: { ...currentGameState.players }, // Start with game's player data
            score: currentGameState.score || {},
            items: currentGameState.items || {}
          };
          
          // Now enhance the player data with our client registry information
          // This ensures pubkeys and connection info are properly included
          this.serverState.clients.forEach((client, clientId) => {
            // Use pubkey as the primary player identifier, fallback to clientId if no pubkey
            const playerKey = client.pubkey && client.pubkey !== '' ? client.pubkey : clientId;
            
            // Get existing player data from game state (could be under pubkey or clientId)
            const existingPlayerData = this.serverState.gameState.players[playerKey] || 
                                     this.serverState.gameState.players[clientId] || 
                                     {};
            
            // Update/create player entry with enhanced connection and identity info
            this.serverState.gameState.players[playerKey] = {
              ...existingPlayerData, // Preserve game data (health, position, etc.)
              connected: client.connected,
              connectionId: client.connectionId,
              clientId: clientId, // Keep track of current connection ID
              pubkey: client.pubkey, // Ensure pubkey is properly transferred
              username: client.username, // Transfer username as well
              lastActive: Date.now(),
              // Ensure basic player properties exist
              id: playerKey,
              name: client.username || existingPlayerData.name || `Player-${clientId}`,
              health: existingPlayerData.health || 100,
              joinTime: existingPlayerData.joinTime || Date.now()
            };
            
            // If we used pubkey as the key but there was data under clientId, remove the clientId entry
            if (playerKey !== clientId && this.serverState.gameState.players[clientId]) {
              delete this.serverState.gameState.players[clientId];
            }
          });
        } else {
          ...
          // Preserve existing players and mark as disconnected
          if (!this.serverState.gameState.players) {
            this.serverState.gameState.players = {};
          }
          
          // First, mark all existing players as disconnected
          Object.keys(this.serverState.gameState.players).forEach(playerKey => {
            if (this.serverState.gameState.players[playerKey]) {
              this.serverState.gameState.players[playerKey].connected = false;
            }
          });
          
          // Add/update state for each connected client
          this.serverState.clients.forEach((client, clientId) => {
            // Use pubkey as the primary player identifier, fallback to clientId if no pubkey
            const playerKey = client.pubkey && client.pubkey !== '' ? client.pubkey : clientId;
        ...
        // Debug: Log all player data to verify pubkey transfer
        Object.keys(this.serverState.gameState.players).forEach(playerKey => {
          const player = this.serverState.gameState.players[playerKey];
          this.log(`[PUBKEY DEBUG] Player ${playerKey}: pubkey=${player.pubkey}, username=${player.username}, clientId=${player.clientId}`);
        });
        ...
        // Create server state info to broadcast separately
        const serverStateInfo = {
          playerCount: Object.keys(this.serverState.gameState.players || {}).length,
          itemCount: Object.keys(this.serverState.gameState.items || {}).length || 0,
          timestamp: Date.now() // Server's current time
        ...
        // Log server state info being created
        this.log(`[SERVER_STATE] Creating server state info: ${JSON.stringify(serverStateInfo)}`);
        
        // Send a dedicated server state update message first
        const serverStateMessage = {
          type: 'server:state:update',
          serverInfo: serverStateInfo
        };
        
        this.log(`[SERVER_STATE] Broadcasting dedicated server state message: ${JSON.stringify(serverStateMessage.serverInfo)}`);
        this.broadcast(serverStateMessage);
        
        // Broadcast game state token if requested
        if (broadcast && token) {
          ...
          const message = {
            ...
            tokenFlow: tokenFlow,
            frame: this.serverState.gameState.frame,
            // Still include serverInfo in game state token for backward compatibility
            serverInfo: serverStateInfo
          };
          
          // Log the full message with stringified serverInfo
          this.log(`[CRITICAL] Broadcasting token message with serverInfo: ${JSON.stringify(message.serverInfo)}`);
  ...
  window.BrowserServer = BrowserServer;
