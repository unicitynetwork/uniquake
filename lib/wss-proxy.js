...
class WSSProxy {
   ...
   * @param {string} options.targetHost - Target game server host (default: localhost)
  ...
  async start() {
    try {
      ...
      this.wss.on('connection', (clientWs, req) => {
        ...
        const targetUrl = `ws://${this.targetHost}:${this.targetPort}`;
      ...
      logger.info(`[WSS Proxy ${this.gameId}] Listening on port ${this.proxyPort} (WSS) -> ${this.targetHost}:${this.targetPort} (WS)`);
      return true;
    } catch (err) {
