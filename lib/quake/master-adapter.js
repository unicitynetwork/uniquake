...
class QuakeMasterAdapter {
  ...
  constructor(config = {}) {
    ...
    // Reference to the main server registry (will be set later)
  ...
  /**
   * Set the server registry reference
   * @param {ServerRegistry} serverRegistry - Main server registry
   */
  setServerRegistry(serverRegistry) {
    this.serverRegistry = serverRegistry;
    console.log('Server registry registered with QuakeJS adapter');
  }
  ...
  handleHeartbeat(conn, data) {
    ...
    // Update heartbeat timestamp in the main server registry if available
    if (this.serverRegistry) {
      const success = this.serverRegistry.updateDedicatedServerHeartbeat(conn.port);
      if (success) {
        console.log(`Updated heartbeat for dedicated server on port ${conn.port}`);
      }
    }
