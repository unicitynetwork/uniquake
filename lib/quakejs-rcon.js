...
class QuakeJSRCON {
  ...
  async getServerStatusDirect(gameId) {
    ...
    try {
      ...
      const rawOutput = await this.gameServerManager.sendRCONCommand(gameId, 'status');
      ...
      return {
        success: true,
        command: 'status',
        output: rawOutput,  // Include raw output for RCON console
  ...
  async getServerInfoDirect(gameId) {
    ...
    try {
      // First get the raw output
      const rawOutput = await this.gameServerManager.sendRCONCommand(gameId, 'serverinfo');
