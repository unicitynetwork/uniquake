        ...
        case 'identity:update':
          ...
          if (message.identity && message.identity.pubkey) {
            ...
            if (client) {
              ...
              console.log(`Updated client identity for ${clientId}: ${client.username} (${client.pubkey.substring(0, 10)}...)`);
...
function handleProxyConnection(message) {
  ...
  if (!serverState.connections.has(clientId)) {
    ...
    const existingClient = serverState.clients.get(clientId);
  ...
  if (!serverState.clients.has(clientId)) {
    ...
    if (connectionId) {
      ...
      if (data && data.identity) {
        ...
        console.log(`Found identity in data: ${identity.username} (${identity.pubkey.substring(0, 10)}...)`);
      }
...
function broadcastMessage(message) {
  ...
  console.log(`Broadcasting message: ${messageStr.length > 160 ? messageStr.substring(0, 160) + '...' : messageStr}`);
  ...
  Collected entry tokens: ${serverState.collectedTokens.length}
    ...
    case 'gameover':
      ...
      try {
        ...
        if (client) {
          console.log(`Winner client info: ${JSON.stringify({
            id: client.id,
            username: client.username,
            hasPubkey: !!client.pubkey,
            pubkeyPreview: client.pubkey ? client.pubkey.substring(0, 10) + '...' : 'null',
