...
class MockServerChannel {
  ...
  receiveMessage(data) {
    if (this.onmessage) {
      console.log(`MockServerChannel for ${this.clientId} received message: ${typeof data} data`);
...
function setupChannel(channel, clientId) {
  ...
  channel.onmessage = (event) => {
    let message;
    try {
      console.log(`Raw channel data from client ${clientId}: ${typeof event.data}`);
      // Safely display data content
      if (typeof event.data === 'string') {
        console.log(`String data (first 100 chars): ${event.data.substring(0, 100)}${event.data.length > 100 ? '...' : ''}`);
      } else if (typeof event.data === 'object') {
        console.log(`Object data: ${JSON.stringify(event.data).substring(0, 100)}...`);
      }
      ...
      if (typeof event.data === 'string') {
        try {
          ...
          console.log(`Successfully parsed string data from client ${clientId}`);
        } catch (parseErr) {
        ...
        message = event.data;
        console.log(`Using object data directly from client ${clientId}`);
      } else {
      ...
      console.log(`Processing message of type: ${message.type} from client ${clientId}`);
      
      // Handle different message types
      switch (message.type) {
        case 'chat':
          // Broadcast chat message to all clients
          console.log(`Broadcasting chat message from ${clientId}: ${message.message}`);
          broadcastMessage({
            type: 'chat',
            from: clientId,
        ...
        default:
          console.log(`Unhandled channel message type: ${message.type} from client ${clientId}`);
          break;
      }
    } catch (err) {
...
function handleProxyData(message) {
  ...
  try {
    console.log(`Data type from client ${clientId}: ${typeof data}`);
    ...
    if (typeof data === 'string') {
      ...
      try {
        ...
        console.log(`Forwarding string data from client ${clientId}`);
        channel.receiveMessage(data);
      }
    } else {
      // If it's already an object, send it directly
      console.log(`Forwarding object data from client ${clientId}`);
      channel.receiveMessage(data);
    }
  } catch (err) {
