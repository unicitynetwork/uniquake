...
async function handleProxyConnection(message) {
  ...
  // Send entry token if token service is available and server pubkey is known
  if (clientState.tokenService && clientState.serverPubkey && !clientState.entryTokenSent) {
    try {
      console.log('Preparing to send entry token to server...');
      
      // Send the token after a short delay to ensure connection is fully established
      setTimeout(async () => {
        try {
  ...
  } else if (clientState.tokenService && !clientState.serverPubkey) {
    console.warn('Cannot send entry token: Server public key not available');
  }
}
