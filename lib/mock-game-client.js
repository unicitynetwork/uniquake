...
function setupChannel() {
  ...
  clientState.channel.onmessage = (event) => {
    try {
      debug(`Received channel message: ${event.data}`);
      const message = JSON.parse(event.data);
      handleChannelMessage(message);
    } catch (err) {
      // If not JSON, treat as raw data
      debug(`Failed to parse message: ${err.message}`);
      debug(`Received raw data: ${event.data}`);
    }
  };
...
function handleChannelMessage(message) {
  switch (message.type) {
    case 'welcome':
      ...
      clientState.serverInfo = message.serverInfo;
    ...
    case 'pong':
      ...
      if (argv.verbose || argv.debug) {
        console.log(`Ping: ${ping}ms (avg: ${Math.round(avg)}ms)`);
      }
      break;
...
function handleProxyData(message) {
  ...
  if (clientState.channel) {
    ...
    if (typeof data === 'string') {
      ...
      try {
        debug(`Forwarding object data to channel: ${JSON.stringify(data).substring(0, 50)}${JSON.stringify(data).length > 50 ? '...' : ''}`);
        // If it's an object, stringify it first
        clientState.channel.receiveMessage(JSON.stringify(data));
      } catch (err) {
        console.error('Failed to stringify data:', err);
      }
    }
  } else {
