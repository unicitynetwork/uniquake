...
class MockClientChannel {
  ...
  receiveMessage(data) {
    if (this.onmessage) {
      console.log(`MockClientChannel received message: ${typeof data} data`);
...
function setupChannel() {
  ...
  console.log('Creating and setting up new MockClientChannel');
  ...
  channel.onmessage = (event) => {
    try {
      console.log(`Received channel message type: ${typeof event.data}`);
      ...
      if (typeof event.data === 'string') {
        try {
          ...
          console.log(`Successfully parsed string message into object`);
        } catch (parseErr) {
        ...
        message = event.data;
        console.log(`Using object message directly`);
      } else {
...
function handleChannelMessage(message) {
  switch (message.type) {
    case 'welcome':
      ...
      console.log(JSON.stringify(clientState.serverInfo, null, 2));
...
function handleProxyData(message) {
  ...
  console.log(`Received proxy data from server ${serverPeerId}`);
    ...
    try {
      ...
      if (typeof data === 'string') {
        ...
        try {
          ...
          console.log(`Forwarding parsed object to channel`);
          clientState.channel.receiveMessage(parsedData);
        } catch (parseErr) {
          // If parsing fails, it's a plain string message
          console.log(`Forwarding string data to channel: ${data.substring(0, 50)}${data.length > 50 ? '...' : ''}`);
          clientState.channel.receiveMessage(data);
        }
      } else {
        // If it's an object, send it directly without stringifying
        console.log(`Forwarding object to channel: ${JSON.stringify(data).substring(0, 50)}...`);
        clientState.channel.receiveMessage(data);
      }
    } catch (err) {
