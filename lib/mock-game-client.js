...
function setupChannel() {
  ...
  clientState.channel.onmessage = (event) => {
    try {
      ...
      console.log(`Successfully parsed message of type: ${message.type}`);
      handleChannelMessage(message);
    } catch (err) {
      // If parsing fails, log the error and raw data
      console.error(`Failed to parse message: ${err.message}`);
      debug(`Raw data: ${JSON.stringify(event.data)}`);
    }
  };
...
function handleProxyData(message) {
  ...
  if (clientState.channel) {
    try {
      ...
      console.log(`Forwarding to channel: ${dataToSend.substring(0, 50)}${dataToSend.length > 50 ? '...' : ''}`);
      
      // Send to channel
      clientState.channel.receiveMessage(dataToSend);
    } catch (err) {
