  ...
  switch (message.type) {
    ...
    case 'game:state:token':
      ...
      try {
        ...
        console.log(`Received game state token for verification at frame ${message.frame || 0}`);
        ...
        if (result.verified) {
          console.log(`Game state verified: Valid state at frame ${result.frame}`);
          if (argv.debug) {
            console.log(`State hash: ${result.stateHash.substring(0, 10)}...`);
          }
        } else if (result.error) {
          console.log(`Game state verification failed: ${result.error}`);
        }
      } catch (error) {
        console.error(`Failed to verify game state token: ${error.message}`);
      }
      break;
...
Game State Verification:
  ...
  State Hash: ${tokenStatus.gameState.stateHash || 'None'}
  `);
    } catch (error) {
      console.log(`
...
async function initializeTokenService() {
  try {
    ...
    clientState.tokenService.debugMode = argv.debug;
