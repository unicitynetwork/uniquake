...
class MasterServer {
  ...
  initializeComponents() {
    ...
    // Initialize PaymentManager (Sphere SDK integration)
    // PaymentManager is null in legacy mode (no mnemonic configured)
    this.paymentManager = null;
    if (process.env.UNIQUAKE_MNEMONIC || process.env.UNIQUAKE_MNEMONIC_FILE) {
      this.paymentManager = new PaymentManager({
        network: process.env.UNIQUAKE_NETWORK || this.config.sphereNetwork || 'testnet',
        defaultPayoutNametag: process.env.UNIQUAKE_DEFAULT_PAYOUT_NAMETAG || 'babaika10',
        entryFee: process.env.UNIQUAKE_ENTRY_FEE || '10',
        entryCoin: process.env.UNIQUAKE_ENTRY_COIN || 'UCT',
        walletUrl: process.env.UNIQUAKE_WALLET_URL || 'https://sphere.unicity.network',
        nostrRelays: (process.env.UNIQUAKE_NOSTR_RELAYS || 'wss://nostr-relay.testnet.unicity.network').split(',').filter(Boolean),
      });
    }
    ...
    if (this.wssServer) {
      ...
      // Set up cross-protocol relay between WS and WSS signaling services
      this.signalingService.setSiblingService(this.secureSignalingService);
      this.secureSignalingService.setSiblingService(this.signalingService);
    }

    // Wire PaymentManager into signaling services
    if (this.paymentManager) {
      this.signalingService.setPaymentManager(this.paymentManager);
      if (this.secureSignalingService) {
        this.secureSignalingService.setPaymentManager(this.paymentManager);
      }
    }
  }
  ...
  async start() {
    try {
      ...
      // Initialize PaymentManager (Sphere SDK)
      if (this.paymentManager) {
        try {
          await this.paymentManager.init();
          logger.info('Sphere payment system initialized');
        } catch (err) {
          logger.error('Sphere payment system failed to initialize: %s', err.message);
          logger.warn('Game server will run without payment features (legacy mode)');
          this.paymentManager = null;
          // Also clear from signaling services
          this.signalingService.setPaymentManager(null);
          if (this.secureSignalingService) {
            this.secureSignalingService.setPaymentManager(null);
          }
        }
      }
  ...
  async stop() {
    ...
    // Destroy PaymentManager
    if (this.paymentManager) {
      await this.paymentManager.destroy();
    }
