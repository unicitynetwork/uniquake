...
const { loadMnemonic, sanitizeConfig, clearMnemonicEnv } = require('./secret-loader');
...
class PaymentManager {
  ...
  constructor(config) {
    ...
    /** @type {Set<NodeJS.Timer>} Active payment timeout timers */
    this._paymentTimers = new Set();
  ...
  async init() {
    ...
    // 10. Clear mnemonic from env now that init succeeded (S3)
    clearMnemonicEnv();

    // 11. Log identity — never log the mnemonic (S3)
  ...
  async _handleIncomingDM(dm) {
    // Reject oversized DM content (prevent memory pressure from malicious payloads)
    if (!dm.content || dm.content.length > 10240) {
      return;
    }
    ...
    if (!msg || typeof msg.type !== 'string' || !msg.type.startsWith('uniquake:')) {
      return;
    }
  ...
  _startPaymentTimeout(sessionId, nametag, invoiceId) {
    ...
    const timer = setInterval(async () => {
      ...
      try {
        ...
        logger.debug('[PaymentManager] Payment poll error for %s: %s', nametag, e.message);
      }

      // T32: Timeout reached — cancel invoice and notify player
      if (elapsed >= PAYMENT_TIMEOUT_MS) {
        clearInterval(timer);
        this._paymentTimers.delete(timer);
        logger.warn(
        ...
        try {
          ...
          logger.debug('[PaymentManager] Timeout notification DM failed: %s', dmErr.message);
        }
      }
    }, PAYMENT_POLL_INTERVAL_MS);

    timer.unref();
    this._paymentTimers.add(timer);
  }
  ...
  async cancelSession(sessionId, reason) {
    ...
    // Already terminal or mid-payout — no action (prevents double-spend)
    if (escrow.state === 'paid_out' || escrow.state === 'cancelled' || escrow.state === 'paying_out') {
      logger.warn('[PaymentManager] Cannot cancel session %s in state %s', sessionId, escrow.state);
      return;
    }
    ...
    try {
      ...
      logger.error('[PaymentManager] State transition failed for cancel of %s: %s', sessionId, e.message);
      return; // Do NOT force-cancel if state machine rejects
    }
  ...
  async destroy() {
    ...
    // Clear all payment timeout timers
    for (const timer of this._paymentTimers) {
      clearInterval(timer);
    }
    this._paymentTimers.clear();
