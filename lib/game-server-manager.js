...
class GameServerManager {
  ...
  async startServer(serverInfo) {
    ...
    try {
      ...
      const serverProcess = childProcess.spawn('node', args, {
        stdio: ['pipe', 'pipe', 'pipe'], // Enable stdin, stdout, stderr pipes for RCON
        detached: false, // Keep process attached for easier management
        cwd: freshQuakeJsPath, // Set working directory to fresh_quakejs
        env: {
          ...process.env,
          TERM: 'xterm', // Set terminal type to make QuakeJS think it has a TTY
          FORCE_COLOR: '1' // Enable color output
        }
      });
      ...
      // Set up RCON response parsing from stdout
      this.setupRCONParsing(serverProcess, gameId);
      ...
      serverProcess.on('disconnect', () => {
        logger.warn(`Server ${gameId} process disconnected from master server (PID: ${serverProcess.pid})`);
        // Update server state to reflect disconnection
        const serverInstance = this.servers.get(gameId);
        if (serverInstance) {
          serverInstance.state = 'disconnected';
        }
      });
  ...
  async sendRCONCommand(gameId, command) {
    ...
    if (!server.process || !server.process.stdin) {
      throw new Error(`Server ${gameId} does not have stdin available`);
    }

    // Check if stdin pipe is still writable (more relevant than 'connected' for stdio pipes)
    if (!server.process.stdin || !server.process.stdin.writable) {
      throw new Error(`Server ${gameId} stdin pipe is not writable (PID: ${server.process.pid})`);
    }

    logger.info(`[RCON DEBUG] Server process exists: ${!!server.process}, PID: ${server.process.pid}, stdin exists: ${!!server.process.stdin}`);
    logger.info(`[RCON DEBUG] Process connected: ${server.process.connected}, killed: ${server.process.killed}, exitCode: ${server.process.exitCode}`);
    logger.info(`[RCON DEBUG] Stdin writable: ${server.process.stdin?.writable}, stdout readable: ${server.process.stdout?.readable}`);
    ...
    return new Promise((resolve, reject) => {
      ...
      // Send command
      try {
        const writeResult = server.process.stdin.write(command + '\n');
        logger.info(`[RCON DEBUG] Sent command to ${gameId}: ${command}, write result: ${writeResult}`);
        ...
        logger.error(`[RCON DEBUG] Failed to send command: ${error.message}`);
  ...
  parseStatusOutput(output) {
    ...
    for (const line of lines) {
      ...
      if (inPlayerData && line.trim() && !line.startsWith('---')) {
  ...
  parsePlayerLine(line) {
    try {
      // Fixed-width parsing based on status command format
      ...
      const lastmsg = parseInt(line.substring(31, 38).trim());
      
      // Find address (variable length)
