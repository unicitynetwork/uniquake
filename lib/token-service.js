...
/*            console.log(`[TokenService] Checking token structure:`, 
    ...
    async verifyGameStateToken(tokenFlow) {
        try {
            console.log(`[TokenService] Verifying game state token...`);
            ...
            console.log(`[TokenService] Token imported successfully, ID: ${token.tokenId.substring(0, 8)}...`);
            
            // For game state tokens, we use null as the secret parameter to check validity without ownership
            const status = await this.TXF.getTokenStatus(token, null, this.transport);
            
            console.log(`[TokenService] Token status check result: valid=${status.valid}`);
            
            if (!status.valid) {
                return { verified: false, error: 'Invalid token status' };
            }
            ...
            if (!stateData.state_hash) {
                console.log(`[TokenService] State data missing required field: state_hash`);
                return { verified: false, error: 'State data missing state hash' };
            }
            
            console.log(`[TokenService] State verification successful, frame: ${stateData.frame || 0}`);
