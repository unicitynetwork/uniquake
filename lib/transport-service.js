...
class TransportService extends EventEmitter {
  ...
  constructor(config, serverRegistry) {
    ...
    // Secure WebSocket server for handling encrypted connections
    this.wssServer = null;
  ...
  /**
   * Set secure WebSocket server reference
   * @param {WebSocketServer} wssServer - Secure WebSocket server instance
   */
  setSecureWebSocketServer(wssServer) {
  ...
  forwardToClient(serverPeerId, clientId, message) {
    ...
    if (!this.proxyConnections.has(serverPeerId)) {
      logger.debug(`No proxy connections found for server ${serverPeerId} when forwarding to client ${clientId}`);
      return false;
    }
    
    const connections = this.proxyConnections.get(serverPeerId);
    if (!connections.has(clientId)) {
      logger.debug(`Client ${clientId} not found in proxy connections for server ${serverPeerId}`);
      return false;
    }
  ...
  cleanupStaleProxyConnections() {
    const now = Date.now();
    const timeout = 7200000; // 2 hours without activity
    let cleanedCount = 0;
