...
class Config {
  constructor() {
    ...
    // Check both SSL_CERT_PATH (app convention) and SSL_CERT_FILE (ssl-manager convention)
    const sslDomain = process.env.SSL_DOMAIN || this.hostIp;
    this.sslCertPath = process.env.SSL_CERT_PATH || process.env.SSL_CERT_FILE || '/etc/letsencrypt/live/' + sslDomain + '/fullchain.pem';
    this.sslKeyPath = process.env.SSL_KEY_PATH || process.env.SSL_KEY_FILE || '/etc/letsencrypt/live/' + sslDomain + '/privkey.pem';
