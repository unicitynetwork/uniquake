...
class ServerRegistry {
  ...
  registerServer(peerId, metadata = {}) {
    ...
    this.servers[id] = {
      ...
      clients: [],
      // Timer tracking for accurate countdown calculations
      timerLastUpdate: null, // When we last received timer data from this server
      lastRestartCountdown: null, // Last restart countdown value received
      lastMatchTimeRemaining: null // Last match time remaining value received
    };
    
    // Initialize timer data if provided during registration
    if (metadata.restartCountdown !== undefined && metadata.restartCountdown !== null) {
      this.servers[id].timerLastUpdate = currentTime;
      this.servers[id].lastRestartCountdown = metadata.restartCountdown;
    }
    
    if (metadata.matchTimeRemaining !== undefined && metadata.matchTimeRemaining !== null) {
      this.servers[id].timerLastUpdate = currentTime;
      this.servers[id].lastMatchTimeRemaining = metadata.matchTimeRemaining;
    }
  ...
  updateServer(peerId, metadata = null) {
    ...
    const currentTime = Date.now();
    this.servers[peerId].lastUpdate = currentTime;
    
    if (metadata) {
      const server = this.servers[peerId];
      
      // Update timer tracking when receiving countdown/timer updates
      if (metadata.restartCountdown !== undefined && metadata.restartCountdown !== null) {
        server.timerLastUpdate = currentTime;
        server.lastRestartCountdown = metadata.restartCountdown;
      }
      
      if (metadata.matchTimeRemaining !== undefined && metadata.matchTimeRemaining !== null) {
        server.timerLastUpdate = currentTime;
        server.lastMatchTimeRemaining = metadata.matchTimeRemaining;
      }
      
      // Reset countdown when server state changes to non-game_over
      if (metadata.state && metadata.state !== 'game_over') {
        server.lastRestartCountdown = null;
      }
      
      // Reset match timer when server state changes to non-running
      if (metadata.state && metadata.state !== 'running') {
        server.lastMatchTimeRemaining = null;
      }
  ...
  getServerList() {
    ...
    return Object.values(this.servers).map(server => {
      // Calculate current timer values based on elapsed time
      const currentTimers = this.calculateCurrentTimers(server, currentTime);
      ...
      return {
        ...
        // Include CURRENT countdown and timer information (calculated, not stale)
        restartCountdown: currentTimers.restartCountdown,
        matchTimeRemaining: currentTimers.matchTimeRemaining
      };
    });
  }
  
  /**
   * Calculate current timer values based on elapsed time since last server update
   * @param {Object} server - Server object
   * @param {number} currentTime - Current timestamp
   * @returns {Object} Object containing current restartCountdown and matchTimeRemaining
   */
  calculateCurrentTimers(server, currentTime) {
    let restartCountdown = null;
    let matchTimeRemaining = null;
    
    // Calculate restart countdown if server is in game_over state
    if (server.dedicatedServerState === 'game_over' && 
        server.timerLastUpdate && 
        server.lastRestartCountdown !== undefined && 
        server.lastRestartCountdown !== null &&
        server.lastRestartCountdown > 0) {
      
      const elapsedSeconds = (currentTime - server.timerLastUpdate) / 1000;
      const currentCountdown = Math.max(0, Math.ceil(server.lastRestartCountdown - elapsedSeconds));
      restartCountdown = currentCountdown > 0 ? currentCountdown : null;
      
      // Safety check: if too much time has elapsed since last update, assume countdown expired
      if (elapsedSeconds > server.lastRestartCountdown + 5) {
        restartCountdown = null;
      }
    }
    
    // Calculate match time remaining if server is running
    if (server.dedicatedServerState === 'running' && 
        server.timerLastUpdate && 
        server.lastMatchTimeRemaining !== undefined && 
        server.lastMatchTimeRemaining !== null &&
        server.lastMatchTimeRemaining > 0) {
      
      const elapsedSeconds = (currentTime - server.timerLastUpdate) / 1000;
      const currentTimeRemaining = Math.max(0, Math.ceil(server.lastMatchTimeRemaining - elapsedSeconds));
      matchTimeRemaining = currentTimeRemaining > 0 ? currentTimeRemaining : null;
      
      // Safety check: if too much time has elapsed since last update, assume timer expired
      if (elapsedSeconds > server.lastMatchTimeRemaining + 5) {
