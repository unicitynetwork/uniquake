...
class ServerRegistry {
  ...
  registerServer(peerId, metadata = {}) {
    ...
    this.servers[id] = {
      ...
      dedicatedServerState: 'not_running', // not_running, starting, running
  ...
  updateGameStateToken(peerId) {
    ...
    console.log(`Updated game state token timestamp for server ${peerId}`);
    return true;
  }
   ...
   * @param {string} peerId - Server peer ID
   * @param {string} state - State: 'not_running', 'starting', 'running'
  ...
  setDedicatedServerState(peerId, state, port = null, gameId = null) {
    ...
    console.log(`Server ${peerId} dedicated server state: ${state}${port ? ` (port ${port})` : ''}${gameId ? ` (gameId ${gameId})` : ''}`);
    return true;
  }
  ...
  updateDedicatedServerHeartbeat(port) {
    ...
    if (!server) {
      console.warn(`No server found for dedicated server port ${port}`);
      return false;
    }
    ...
    if (server.dedicatedServerState === 'starting') {
      ...
      console.log(`Server ${server.peerId} dedicated server transitioned to running state (first heartbeat from port ${port})`);
    }
  ...
  checkDedicatedServerHeartbeats(heartbeatTimeout = 400000) {
    ...
    Object.values(this.servers).forEach(server => {
      // Only check servers that should have dedicated servers
      if (server.dedicatedServerPort && server.dedicatedServerState === 'running') {
        if (server.lastHeartbeat && (now - server.lastHeartbeat > heartbeatTimeout)) {
          server.dedicatedServerState = 'not_running';
          console.log(`Server ${server.peerId} dedicated server marked as not_running (no heartbeat for ${Math.round((now - server.lastHeartbeat) / 1000)}s)`);
          staleCount++;
        }
      }
    });
    ...
    if (staleCount > 0) {
      console.log(`Marked ${staleCount} dedicated servers as not_running due to stale heartbeats`);
    }
  }
  ...
  pruneInactiveServers(maxAge = 7200000) {
    ...
    if (pruneCount > 0) {
      console.log(`[SESSION TIMEOUT] Pruned ${pruneCount} servers after 2-hour session timeout. ${Object.keys(this.servers).length} servers remain.`);
    }
