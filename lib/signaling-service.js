...
class SignalingService {
  ...
  handleSignalingMessage(clientId, message) {
    ...
    switch (message.type) {
      ...
      case 'join_session':
        // Forward to PaymentManager if available (hybrid DM/WS model)
        if (this.paymentManager) {
          this.paymentManager._handleJoinRequest(
            message.nametag || (client && client.nametag) || clientId,
    ...
    for (const [gameId, server] of this.gameServerManager.servers.entries()) {
      if (server.peerId === serverPeerId) {
        if (server.playerCache && server.playerCache.players) {
          for (const [name, playerData] of server.playerCache.players.entries()) {
            scores.push({
              name: playerData.name || name,
              score: playerData.score || 0,
              // Bots in Quake3 typically have names like "Sarge", "Keel", etc.
              // Player_ prefix indicates a real player connected via browser
              isBot: !(name.startsWith('Player_') || name.startsWith('player_')),
            });
          }
        }
        break;
