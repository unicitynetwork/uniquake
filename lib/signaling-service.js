  ...
  async handleStopGameServer(clientId, message) {
    ...
    try {
      ...
      if (message.isRestartCycle) {
        ...
        if (serverPeerId) {
          console.log(`Setting server ${serverPeerId} to game_over state for restart cycle (preserving address)`);
          ...
          console.log(`Server ${serverPeerId} set to game_over state (address preserved for restart)`);
        }
      } else {
        // For normal stops, remove the server completely
        if (serverPeerId) {
          console.log(`Removing server ${serverPeerId} completely (normal stop)`);
