...
class SignalingService {
  ...
  constructor(wsServer, serverRegistry, credentialManager, transportService, masterServer) {
    ...
    // Reference to sibling signaling service (for cross-protocol message relay)
    this.siblingService = null;
    ...
    // Reference to the PaymentManager (will be set later, may remain null in legacy mode)
  ...
  /**
   * Set the PaymentManager reference for Sphere SDK integration.
   * When set, enables admission gating, session lifecycle hooks,
   * and payment-based join flows. When null (legacy mode), all
   * payment-related functionality is bypassed.
   * @param {PaymentManager} paymentManager - The payment manager instance
   */
  setPaymentManager(paymentManager) {
    this.paymentManager = paymentManager;
    console.log('PaymentManager set for signaling service');
  }
  
  /**
   * Set the sibling signaling service for cross-protocol message relay
   * @param {SignalingService} siblingService - The sibling signaling service
   */
  setSiblingService(siblingService) {
    this.siblingService = siblingService;
    console.log('Sibling signaling service set for cross-protocol relay');
  }
   ...
   */
  start() {
    // If the master server has a Quake handler, pass our WebSocket server to it
    if (this.master && this.master.quakeHandler && this.master.quakeHandler.useWebSocketServer) {
      this.master.quakeHandler.useWebSocketServer(this.wsServer);
    }
    ...
    this.wsServer.on('connection', (ws, req) => {
      ...
      // Parse query params from the HTTP upgrade request for admission gating
      let sessionParam = null;
      let admissionToken = null;
      try {
        const url = new URL(req.url, 'http://localhost');
        sessionParam = url.searchParams.get('session');
        admissionToken = url.searchParams.get('admissionToken');
      } catch (e) {
        // URL parsing failed — proceed without admission params (legacy mode)
      }

      // Admission gate (S1): validate admission token if present and PaymentManager is active
      let admittedNametag = null;
      let admittedRole = null;
      if (sessionParam && admissionToken && this.paymentManager) {
        const tokenResult = this.paymentManager.validateAdmissionToken(sessionParam, admissionToken);
        if (!tokenResult) {
          // Invalid or expired admission token — reject connection
          ws.close(4001, 'Invalid admission token');
          return;
        }
        // S2: Identity binding — use nametag from validated token, not self-asserted
        admittedNametag = tokenResult.nametag;
        admittedRole = tokenResult.role;
      }
      ...
      // Store admission data on the client record if validated
      if (admittedNametag) {
        const client = this.clients.get(clientId);
        if (client) {
          client.nametag = admittedNametag;
          client.role = admittedRole;
          client.sphereSession = sessionParam;
        }
      }

      // Track nametag-to-clientId mapping for score resolution
      if (admittedNametag && sessionParam) {
        if (!this._nametagToClient) this._nametagToClient = new Map();
        this._nametagToClient.set(admittedNametag, clientId);
      }
  ...
  /**
   * Check if a message should be relayed to sibling service
   * @param {Object} data - Message data
   * @returns {boolean} True if message should be relayed
   */
  shouldRelayMessage(data) {
    if (!data || typeof data !== 'object') return false;
    
    // Relay score updates, match time updates, and match end messages
    const relayTypes = [
      'player_score_update',
      'server:match:time',
      'match:end',
    ...
    return relayTypes.includes(data.type);
  }
  ...
  handleSignalingMessage(clientId, message) {
    ...
    switch (message.type) {
      ...
      case 'broadcast_to_all':
        this.handleBroadcastToAll(clientId, message);
        break;
      ...
      case 'join_session':
        // Forward to PaymentManager if available
        // S2: Use verified nametag from admission gate (client.nametag), NOT the
        // self-asserted message.nametag. If client has no verified nametag
        // (connected without admission token), reject the join request.
        if (this.paymentManager) {
          const verifiedNametag = client && client.nametag;
          if (!verifiedNametag) {
            // No verified identity — player must join via DM first
            this.sendToClient(clientId, {
              type: 'error',
              message: 'Join via Sphere wallet DM first. WebSocket join requires verified identity.',
            });
            break;
          }
          this.paymentManager._handleJoinRequest(
            verifiedNametag,
            { sessionId: message.sessionId, role: message.role || 'player' }
          );
        }
        break;

      case 'request_scores':
        this.handleScoreRequest(clientId, message);
        break;
  ...
  handleConnectToServer(clientId, message) {
    ...
    let serverClient = this.findClientByPeerId(targetPeerId);
    
    // If not found in this service, check sibling service
    if (!serverClient && this.siblingService) {
      serverClient = this.siblingService.findClientByPeerId(targetPeerId);
      if (serverClient) {
        console.log(`[CROSS-PROTOCOL] Server ${targetPeerId} found in sibling service`);
      }
    }
    ...
    if (message.useWebSocket) {
      ...
      let success = this.transportService.createProxyConnection(clientId, targetPeerId);
      
      // If failed and server is in sibling service, create cross-protocol proxy
      if (!success && this.siblingService && this.siblingService.findClientByPeerId(targetPeerId)) {
        console.log(`[CROSS-PROTOCOL] Creating cross-protocol proxy for client ${clientId} to server ${targetPeerId}`);
        
        // Create proxy connection info in both transport services
        const connectionId = `${clientId}-${targetPeerId}-${Date.now()}`;
        
        // Add to this transport service
        if (!this.transportService.proxyConnections.has(targetPeerId)) {
          this.transportService.proxyConnections.set(targetPeerId, new Map());
        }
        const localProxies = this.transportService.proxyConnections.get(targetPeerId);
        localProxies.set(clientId, {
        ...
        // Add to sibling transport service
        if (!this.siblingService.transportService.proxyConnections.has(targetPeerId)) {
          this.siblingService.transportService.proxyConnections.set(targetPeerId, new Map());
        }
        const siblingProxies = this.siblingService.transportService.proxyConnections.get(targetPeerId);
        siblingProxies.set(clientId, {
          connectionId: connectionId,
          clientId: clientId,
          serverPeerId: targetPeerId,
          serverId: serverClient.id,
        ...
        // Also update the server's connectedTo list in sibling service
        if (!serverClient.connectedTo) {
          serverClient.connectedTo = [];
        }
        if (!serverClient.connectedTo.includes(clientId)) {
          serverClient.connectedTo.push(clientId);
        }
        
        // Update server player count
        this.serverRegistry.addClientToServer(targetPeerId, clientId);
      }
      ...
      if (success) {
        ...
        // Notify server about the connection through sibling service if needed
        if (this.siblingService && this.siblingService.findClientByPeerId(targetPeerId)) {
          this.siblingService.sendToClient(serverClient.id, {
            type: 'proxy_connection',
            clientId: clientId,
            connectionId: `${clientId}-${targetPeerId}-${Date.now()}`
          });
        }
  ...
  handleProxyDataFromServer(clientId, message) {
    ...
    // Check if this is a broadcast message that should be relayed to sibling service
    const shouldRelay = this.shouldRelayMessage(data);
    
    // Forward the data to the client through transport service
    if (this.transportService) {
      const forwarded = this.transportService.forwardToClient(server.peerId, targetClientId, data);
      
      // If the client wasn't found in this service and we should relay, try the sibling
      if (!forwarded && shouldRelay && this.siblingService) {
        console.log(`[RELAY] Client ${targetClientId} not found in this service, relaying to sibling service`);
        
        // Check if the sibling service has this client
        if (this.siblingService.clients.has(targetClientId)) {
          // Forward through the sibling's transport service
          this.siblingService.transportService.forwardToClient(server.peerId, targetClientId, data);
          console.log(`[RELAY] Successfully relayed message to client ${targetClientId} via sibling service`);
        }
      }
    }
  }
  ...
  async handleStartGameServer(clientId, message) {
    ...
    try {
      ...
      if (client && client.isServer && client.peerId) {
        ...
        // Use proxy address if available, otherwise use direct address
        const serverAddress = serverInstance.serverInfo.proxyAddress || serverInstance.serverInfo.address;
        ...
        this.serverRegistry.updateServer(serverPeerId, {
          ...
          directAddress: serverInstance.serverInfo.address,
          proxyAddress: serverInstance.serverInfo.proxyAddress
        });
        ...
        // Use proxy address if available
        const serverAddress = serverInstance.serverInfo.proxyAddress || serverInstance.serverInfo.address;
      ...
      // Use proxy address if available
      const responseAddress = serverInstance.serverInfo.proxyAddress || serverInstance.serverInfo.address;
      ...
      this.sendToClient(clientId, {
        ...
        serverInfo: {
          ...
          directAddress: serverInstance.serverInfo.address,
          proxyAddress: serverInstance.serverInfo.proxyAddress,
          useWSS: !!serverInstance.serverInfo.proxyAddress,
          map: serverInstance.serverInfo.map,
          maxPlayers: serverInstance.serverInfo.maxPlayers
        },
        success: true
      ...
      // Create a payment session if PaymentManager is active
      if (this.paymentManager) {
        try {
          const client = this.clients.get(clientId);
          this.paymentManager.createSession({
            sessionId: gameId,
            creatorNametag: (client && client.nametag) || clientId,
            isDefaultSession: !!(message.isDefaultSession || message.isHealthCheck),
          });
        } catch (pmErr) {
          console.error(`[PaymentManager] Failed to create session for ${gameId}:`, pmErr.message);
          // Non-fatal — game server still runs in legacy mode
        }
      }
    } catch (error) {
  ...
  async handleStopGameServer(clientId, message) {
    ...
    try {
      ...
      if (message.isRestartCycle) {
        ...
        // Distribute winnings on match end (game_over = restart cycle)
        if (this.paymentManager) {
          try {
            const scores = this.getGameScores(serverPeerId);
            this.paymentManager.distributeWinnings(message.gameId, scores)
              .catch(err => console.error(`[PaymentManager] distributeWinnings failed for ${message.gameId}:`, err.message));
          } catch (pmErr) {
            console.error(`[PaymentManager] Failed to distribute winnings for ${message.gameId}:`, pmErr.message);
          }
        }
      } else {
        ...
        // Cancel session on normal stop (manual "End Match")
        if (this.paymentManager) {
          try {
            this.paymentManager.cancelSession(message.gameId, 'Server stopped')
              .catch(err => console.error(`[PaymentManager] cancelSession failed for ${message.gameId}:`, err.message));
          } catch (pmErr) {
            console.error(`[PaymentManager] Failed to cancel session for ${message.gameId}:`, pmErr.message);
          }
        }
      }
  ...
  /**
   * Handle client request for player scores from a specific game server
   * @param {string} clientId - Client ID requesting scores
   * @param {Object} message - Score request message
   */
  async handleScoreRequest(clientId, message) {
    // Client requested player scores from a specific game server
    ...
    try {
      // Validate input
      if (!message.gameId && !message.peerId) {
        return this.sendToClient(clientId, {
          type: 'scores_response',
          requestId: message.requestId,
          error: 'Missing game server ID or peer ID'
        });
      }
      
      // Get gameId from peerId if needed
      let gameId = message.gameId;
      if (!gameId && message.peerId) {
        // Find the game server by peer ID
        const serverInfo = this.serverRegistry.getServer(message.peerId);
        if (serverInfo && serverInfo.gameId) {
          gameId = serverInfo.gameId;
        } else {
          return this.sendToClient(clientId, {
            type: 'scores_response',
            requestId: message.requestId,
            error: `Game server not found for peer ID: ${message.peerId}`
          });
        }
      }
      
      // Get player scores using RCON status command
      const statusResult = await this.quakeJSRCON.getServerStatusDirect(gameId);
      
      // Extract player scores and create leaderboard
      const allScores = statusResult.players.map((player, index) => ({
        name: player.name,
        score: player.score || 0,
        ping: player.ping || 999,
        rank: index + 1 // Simple rank based on order returned by server
      }));
      
      // Sort by score descending for proper ranking
      ...
      // Find the requesting client's score if they provided a player name
      let clientScore = null;
      if (message.playerName) {
        clientScore = allScores.find(player => 
          player.name.toLowerCase() === message.playerName.toLowerCase()
        );
      }
      
      // Calculate match time and countdown info if available
      let matchInfo = null;
      try {
        // Try to get additional server info for match timing
        const serverInfoResult = await this.quakeJSRCON.getServerInfoDirect(gameId);
        matchInfo = {
          map: statusResult.map || 'Unknown',
          playerCount: allScores.length,
          maxPlayers: statusResult.maxPlayers || 16,
          uptime: serverInfoResult.uptime || 0
        };
      } catch (infoError) {
        console.warn(`Could not get additional server info for ${gameId}:`, infoError.message);
        matchInfo = {
          map: statusResult.map || 'Unknown',
          playerCount: allScores.length,
          maxPlayers: statusResult.maxPlayers || 16,
      ...
      console.log(`[SCORES] Sent score data for server ${gameId} to client ${clientId}: ${allScores.length} players`);
  ...
  /**
   * Handle broadcast to all clients including cross-protocol relay
   * @param {string} clientId - Server's client ID sending the broadcast
   * @param {Object} message - Broadcast message
   */
  handleBroadcastToAll(clientId, message) {
    const server = this.clients.get(clientId);
    if (!server || !server.isServer) {
      console.warn(`Broadcast request from non-server client ${clientId}`);
      return;
    }
    ...
    console.log(`[BROADCAST] Server ${server.peerId} broadcasting ${data.type} to all clients`);
    
    // Get all connected client IDs from this server
    const connectedClients = server.connectedTo || [];
    
    // Broadcast to clients connected to this signaling service
    let localCount = 0;
    for (const targetClientId of connectedClients) {
    ...
    console.log(`[BROADCAST] Sent to ${localCount} clients on this service`);
    
    // Relay to sibling service for cross-protocol broadcast
    if (this.siblingService && this.shouldRelayMessage(data)) {
      let siblingCount = 0;
      for (const targetClientId of connectedClients) {
        if (this.siblingService.clients.has(targetClientId)) {
          // Forward through sibling's transport service
          this.siblingService.transportService.forwardToClient(server.peerId, targetClientId, data);
          siblingCount++;
        }
      }
      ...
      if (siblingCount > 0) {
        console.log(`[BROADCAST] Relayed to ${siblingCount} clients on sibling service`);
      }
    }
  }

  /**
   * Extract player scores from the game server manager's player cache.
   * Used by the PaymentManager to determine match winner for winnings distribution.
   *
   * @param {string} serverPeerId - Server peer ID
   * @returns {Array<{ name: string, score: number, isBot: boolean }>} Player scores
   */
  getGameScores(serverPeerId) {
    const scores = [];
    ...
    // Build a reverse map: clientId -> nametag from our tracking
    const clientToNametag = new Map();
    if (this._nametagToClient) {
      for (const [nametag, clientId] of this._nametagToClient) {
        clientToNametag.set(clientId, nametag);
      }
    }

    // Also build a set of connected human client IDs for bot detection
    const humanClientIds = new Set();
    for (const [clientId, client] of this.clients) {
      if (client.nametag) {
        humanClientIds.add(clientId);
      }
    }

    // Find the game server by peer ID to access its player cache
    for (const [gameId, server] of this.gameServerManager.servers.entries()) {
      if (server.peerId === serverPeerId) {
        if (server.playerCache && server.playerCache.players) {
          for (const [slot, playerData] of server.playerCache.players.entries()) {
            const quakeName = playerData.name || slot;

            // Determine if this is a bot or human player:
            // Humans connect via WebSocket/WebRTC (tracked in clients map)
            // Bots are spawned by the Quake engine (no client connection)
            const clientId = playerData.clientId || null;
            const isBot = !clientId || !humanClientIds.has(clientId);

            // Map Quake player name to nametag for human players
            // This is CRITICAL for payout — the winner filter uses nametags
            const nametag = clientId ? (clientToNametag.get(clientId) || quakeName) : quakeName;

            scores.push({
              name: nametag,  // Use nametag for humans, Quake name for bots
