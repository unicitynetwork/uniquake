...
class SignalingService {
  ...
  startTokenMonitoring() {
    ...
    console.log('Token monitoring started - checking every 30 seconds');
  }
  ...
  checkInactiveGameServers() {
    ...
    console.log(`[TOKEN MONITOR] Found ${inactiveServers.length} servers that haven't sent game state tokens in over 1 minute`);
    console.log(`[TOKEN MONITOR] These servers will be terminated for token inactivity, separate from 2-hour session timeout`);
  ...
  async terminateInactiveServer(peerId) {
    console.log(`[TOKEN MONITOR] Terminating server ${peerId} - no game state tokens received for 1 minute`);
    console.log(`[TOKEN MONITOR] This is token-based termination, not session timeout (which is 2 hours)`);
    ...
    if (this.gameServerManager) {
      try {
        ...
        console.log(`Successfully terminated dedicated server process for ${peerId}`);
      } catch (error) {
  ...
  handleClientDisconnection(clientId) {
    ...
    if (!client) {
      console.log(`Client ${clientId} already removed from registry`);
      return; // Client already removed
    }
    
    console.log(`[DISCONNECT] Client ${clientId} disconnected - searching for connected servers`);
    console.log(`[DISCONNECT] Total clients in registry: ${this.clients.size}`);
    
    // Debug: List all clients and their types
    let serverCount = 0;
    for (const [id, clientData] of this.clients.entries()) {
      if (clientData.isServer) {
        serverCount++;
        console.log(`[DISCONNECT] Server ${id} (peerId: ${clientData.peerId}) has connectedTo: [${clientData.connectedTo ? clientData.connectedTo.join(', ') : 'undefined'}]`);
      }
    }
    console.log(`[DISCONNECT] Found ${serverCount} servers total`);
    ...
    for (const [serverId, serverClient] of this.clients.entries()) {
      if (serverClient.isServer) {
        ...
        console.log(`[DISCONNECT] Checking server ${serverId}: wasConnected=${wasConnected}, connectedTo=${serverClient.connectedTo ? serverClient.connectedTo.join(',') : 'undefined'}`);
        
        if (wasConnected) {
          console.log(`[DISCONNECT] ✅ Notifying server ${serverId} about client ${clientId} disconnection`);
    ...
    if (notifiedServers === 0) {
      console.log(`No servers found for disconnected client ${clientId} - checking transport service connections`);
      ...
      if (this.transportService && this.transportService.clientConnections) {
        ...
        if (clientConnections) {
          for (const serverPeerId of clientConnections) {
            ...
            if (serverClient) {
              console.log(`Found server ${serverClient.id} via transport service - notifying about client ${clientId} disconnection`);
    ...
    console.log(`Notified ${notifiedServers} servers about client ${clientId} disconnection`);
  ...
  handleProxyConnectionFromServer(clientId, message) {
    ...
    console.log(`[PROXY_CONN] Received proxy_connection from ${clientId} for target client ${targetClientId}`);
    
    // Find server's peer ID
    const server = this.clients.get(clientId);
    if (!server || !server.isServer || !server.peerId) {
      console.warn(`[PROXY_CONN] ❌ Received proxy_connection from non-server client: ${clientId}`);
      console.warn(`[PROXY_CONN] Client data: isServer=${server?.isServer}, peerId=${server?.peerId}`);
      return;
    }
    
    console.log(`[PROXY_CONN] ✅ Server ${server.peerId} (clientId: ${clientId}) establishing proxy connection with client ${targetClientId}`);
    
    // Track that this client is connected to this server
    if (!server.connectedTo) {
      server.connectedTo = [];
      console.log(`[PROXY_CONN] Created new connectedTo array for server ${server.peerId}`);
    }
    if (!server.connectedTo.includes(targetClientId)) {
      server.connectedTo.push(targetClientId);
      console.log(`[PROXY_CONN] ✅ Added client ${targetClientId} to server ${server.peerId} connection list. Total connected: ${server.connectedTo.length}`);
      console.log(`[PROXY_CONN] Server ${server.peerId} connectedTo list: [${server.connectedTo.join(', ')}]`);
    } else {
      console.log(`[PROXY_CONN] Client ${targetClientId} already in server ${server.peerId} connection list`);
    }
  ...
  handleDisconnectFromServer(clientId, message) {
    ...
    console.log(`[GRACEFUL_DISCONNECT] Client ${clientId} gracefully disconnecting from server ${serverPeerId}`);
    
    // Find the server and notify it about the client disconnection
    const serverClient = this.findClientByPeerId(serverPeerId);
    if (serverClient) {
      console.log(`[GRACEFUL_DISCONNECT] Notifying server ${serverClient.id} about client ${clientId} graceful disconnection`);
      ...
      if (serverClient.connectedTo && serverClient.connectedTo.includes(clientId)) {
        ...
        if (index > -1) {
          serverClient.connectedTo.splice(index, 1);
          console.log(`[GRACEFUL_DISCONNECT] Removed client ${clientId} from server ${serverPeerId} connection list`);
        }
      }
    } else {
      console.warn(`[GRACEFUL_DISCONNECT] Server with peerId ${serverPeerId} not found for client ${clientId} disconnection`);
    }
    ...
    console.log(`[GRACEFUL_DISCONNECT] Client ${clientId} disconnected from game server but remains connected to master`);
  }
  ...
  handleServerStateUpdate(clientId, message) {
    ...
    if (!client || !client.isServer || !client.peerId) {
      console.warn(`Received server:state:update from non-server client: ${clientId}`);
      return;
    }
    ...
    if (success) {
      console.log(`Game state token received from server ${client.peerId}`);
      ...
      console.warn(`Failed to update game state token for server ${client.peerId}`);
    }
  }
  ...
  handleUpdateServer(clientId, message) {
    const client = this.clients.get(clientId);
    if (!client || !client.isServer || !client.peerId) {
      console.warn(`Received update_server from non-server client: ${clientId}`);
      return;
    }
    
    console.log(`Server ${client.peerId} updating server info:`, message.serverInfo);
    ...
    if (success) {
      console.log(`Server ${client.peerId} information updated successfully`);
      ...
      console.warn(`Failed to update server ${client.peerId} information`);
      this.sendToClient(clientId, {
        type: 'server_updated',
        success: false,
        peerId: client.peerId,
  ...
  broadcastServerListUpdate() {
    ...
    console.log(`Broadcasting server list update to ${this.clients.size} clients`);
  ...
  async handleStartGameServer(clientId, message) {
    ...
    try {
      ...
      if (client && client.isServer && client.peerId) {
        ...
        console.log(`Updated server ${serverPeerId} with dedicated server address ${serverInstance.serverInfo.address}`);
      } else {
        // Fallback in case we don't have a registered server (shouldn't happen)
        serverPeerId = `dedicated-${gameId}`;
        console.log(`No existing server ID found, creating new ID: ${serverPeerId}`);
        ...
        console.log(`Registered dedicated server ${serverPeerId} with address ${serverInstance.serverInfo.address}`);
      }
  ...
  async handleStopGameServer(clientId, message) {
    ...
    try {
      ...
      // If we found a server peer ID, update its info to remove the dedicated server address
      if (serverPeerId) {
        console.log(`Updating server ${serverPeerId} to remove dedicated server information`);
        
        // If the client is still connected, use its information
        if (client && client.isServer && client.serverInfo) {
          // Update the server with client's original info, but remove address
          const updatedInfo = { ...client.serverInfo };
          delete updatedInfo.address;  // Remove the dedicated server address
          delete updatedInfo.isDedicated;  // Remove the dedicated flag
          
          this.serverRegistry.updateServer(serverPeerId, updatedInfo);
          console.log(`Updated server ${serverPeerId} with original client information`);
        } else {
          // If we don't have client info, just update to remove dedicated fields
          this.serverRegistry.updateServer(serverPeerId, {
            address: null,
            isDedicated: false
          });
        ...
        console.log(`Removed dedicated server ${dedicatedServerId} from registry (fallback)`);
      }
  ...
  handleListServers(clientId, message) {
    console.log(`Client ${clientId} requested server list`);
  ...
  async handleRCONCommand(clientId, message) {
    console.log(`Client ${clientId} requested RCON command: ${message.command} on server ${message.gameId}`);
  ...
  handleRCONCommands(clientId, message) {
    console.log(`Client ${clientId} requested available RCON commands`);
